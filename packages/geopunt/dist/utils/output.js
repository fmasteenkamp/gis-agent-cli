...
import { toWkt, toGeoJsonFeature, toGoogleMapsUrl, featureCollectionToKml, transformFeatureCollection } from '@gis/shared';
// --- Helper: apply CRS transform to a FeatureCollection if needed ---
function maybeToCrs(fc, crs) {
    return crs === '4326' ? transformFeatureCollection(fc, 'EPSG:4326') : fc;
}
// --- Search results ---
export function renderCaPaKeyResults(results, format, crs = '31370') {
...
// --- Lookup result ---
export function renderCaPaKeyLookup(result, format) {
    if (format === 'geojson' || format === 'kml') {
        if (!result.geometry) {
    ...
    table.push({ [chalk.bold('CaPaKey')]: chalk.cyan(result.capaKey) }, { [chalk.bold('Municipality')]: result.municipality }, { [chalk.bold('Department')]: result.department }, { [chalk.bold('Section')]: result.section }, { [chalk.bold('Perceelnr')]: result.perceelnummer }, { [chalk.bold('Grondnummer')]: result.grondnummer }, { [chalk.bold('Addresses')]: result.addresses.length > 0 ? result.addresses.join('\n') : chalk.gray('none') });
    if (result.centroid) {
...
// --- Reverse results ---
export function renderReverseResults(results, format, crs = '31370') {
    if (format === 'geojson' || format === 'kml') {
...
// --- Suggestions ---
export function renderSuggestions(suggestions, format) {
    if (format === 'json') {
...
// --- Batch results ---
export function renderBatchResults(batch, format, crs = '31370') {
    if (format === 'geojson' || format === 'kml') {
        const features = batch.flatMap(b => b.results.filter(r => r.geometry).map(r => {
            const { geometry, ...properties } = r;
