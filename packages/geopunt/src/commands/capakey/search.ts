...
const VALID_FORMATS = ['table', 'json', 'geojson', 'wkt', 'kml', 'url'] as const;

function validateFormat(format: string, allowed: readonly string[] = VALID_FORMATS): OutputFormat {
...
function validateCrs(crs: string): CRS {
...
export function buildAddressCommand(): Command {
  ...
  address
    ...
    .action(async (addr: string, opts: { results: string; format: string; coordinates?: boolean; geometry?: boolean; crs: string }) => {
  ...
  address
    ...
    .action(async (query: string, opts: { results: string; format: string }) => {
      ...
      try {
        ...
        renderSuggestions(suggestions, format as 'table' | 'json');
      } catch (err) {
        console.error(chalk.red((err as Error).message));
        process.exit(1);
      }
  ...
  address
    ...
    .action(async (c1: string, c2: string, opts: { results: string; format: string; crs: string }) => {
  ...
  address
    ...
    .action(async (addr: string, opts: { format: string }) => {
      const format = validateFormat(opts.format);
  ...
  address
    ...
    .option('-c, --coordinates', 'Include coordinates in output')
    .option('-g, --geometry', 'Include parcel geometry')
    .action(async (file: string, opts: { format: string; crs: string; coordinates?: boolean; geometry?: boolean }) => {
      const format = validateFormat(opts.format);
      const crs = validateCrs(opts.crs);
      ...
      try {
        const batch: { query: string; results: import('../../models/geopunt.js').CaPaKeySearchResult[] }[] = [];
  ...
  address
    ...
    .option('-f, --format <format>', 'Output format: table|json', 'table')
    .option('--crs <crs>', 'Coordinate system: 31370 or 4326', '31370')
    .action(async (opts: { format: string; crs: string }) => {
      const format = validateFormat(opts.format, ['table', 'json']);
      const crs = validateCrs(opts.crs);
