...
import { createBufferPolygon, toWkt, featureCollectionToKml, transformFeatureCollection } from '@gis/shared';
import type { CRS } from '../models/geopunt.js';

export function buildBufferCommand(): Command {
  return new Command('buffer')
    ...
    .action(async (location: string, radiusStr: string, opts: { format: string; crs: string }) => {
      ...
      try {
        ...
        if (isCoords) {
          ...
          if (crs === '4326') {
            const { wgs84ToLambert72 } = await import('@gis/shared');
        ...
        switch (format) {
          case 'wkt':
            ...
            break;
          case 'kml': {
            let fc: unknown = { type: 'FeatureCollection', features: [{ type: 'Feature', geometry: bufferGeom, properties: { radius, center_x: x, center_y: y } }] };
