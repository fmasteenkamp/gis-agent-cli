...
import { wgs84ToLambert72, lambert72ToWgs84, transformGeometry } from '@gis/shared';
import type { CRS } from '../models/geopunt.js';

const factory = new GeometryFactory();
const reader = new GeoJSONReader(factory);

export function buildDistanceCommand(): Command {
  return new Command('distance')
    ...
    .action(async (locationA: string, locationB: string, opts: { format: string; crs: string }) => {
      ...
      try {
        ...
        if (opts.format === 'json') {
          ...
          if (labelA) result.locationA = labelA;
          if (labelB) result.locationB = labelB;
          console.log(JSON.stringify(result, null, 2));
        } else {
          if (labelA) console.log(chalk.gray(`A: ${labelA}`));
          if (labelB) console.log(chalk.gray(`B: ${labelB}`));
          console.log();
...
function toWgs84Obj(pt: { x: number; y: number }): { lat: number; lon: number } {
  const [lon, lat] = lambert72ToWgs84(pt.x, pt.y);
  return { lat: Math.round(lat * 1e6) / 1e6, lon: Math.round(lon * 1e6) / 1e6 };
}

type JstsGeometry = ReturnType<typeof reader.read>;
...
async function resolveGeometry(
  ...
  label: string,
): Promise<[JstsGeometry, string | null]> {
  // @file
...
function parseGeoJsonGeometry(raw: string, crs: string): JstsGeometry {
  let parsed: unknown;
  try {
  ...
  if (obj.type === 'Feature') {
    ...
    const fc = parsed as { features: { geometry: unknown }[] };
