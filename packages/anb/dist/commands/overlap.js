...
import { geocodeAddress, wgs84ToLambert72, createBufferPolygon, transformGeometry } from '@gis/shared';
...
export function buildOverlapCommand() {
        ...
        try {
            ...
            for (const cfg of layers) {
                ...
                try {
                    ...
                    allResults.push({ keyword: cfg.keyword, label: cfg.label, result: { error: err.message } });
                }
            }
            spinner.stop();
            // Output
            ...
            for (const r of allResults) {
                ...
                const intersecting = data.intersecting;
...
function parseGeoJsonInput(location, crs) {
    let raw = null;
    // @file.geojson
...
function validateOverlapFeature(feature, crs) {
    const issues = [];
    ...
    if (issues.length === 0) {
        try {
            ...
            issues.push(`geometry parse error: ${err.message}`);
        }
    }
    return issues;
}
function extractFirstCoord(coords) {
...
function printLayerList(layers = OVERLAP_LAYERS) {
    const table = new Table({
