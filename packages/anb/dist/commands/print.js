...
import { geocodeAddress, wgs84ToLambert72, transformGeometry, fromWkt } from '@gis/shared';
import { exportWebMap, getParcelExtent, geojsonToFeatureCollectionLayer, } from '../services/print.js';
...
async function resolveMapServerLayer(url, spinner) {
...
function parseHighlightInput(input, crs) {
    let raw;
    // @file — read from disk
    if (input.startsWith('@')) {
    ...
    if (isWkt(raw)) {
        ...
        try {
            ...
            console.error(chalk.red(`Error: invalid WKT — ${err.message}`));
            process.exit(1);
        }
    }
    else {
