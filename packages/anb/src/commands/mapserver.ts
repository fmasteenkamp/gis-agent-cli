...
function resolveUrl(layerOrUrl: string): string {
  if (layerOrUrl.startsWith('http')) return layerOrUrl;
...
export function buildMapServerCommand(): Command {
  const cmd = new Command('mapserver')
    ...
    .action((opts: { list?: boolean; search?: string; service?: string }) => {
      if (opts.list || opts.service) {
        let layers = opts.service ? filterByService(opts.service) : MAPSERVER_LAYERS;
        if (opts.search) {
          const q = opts.search.toLowerCase();
          layers = layers.filter(l =>
            l.keyword.includes(q) || l.label.toLowerCase().includes(q) || l.description.toLowerCase().includes(q)
          );
        }
        if (layers.length === 0) {
  ...
  cmd
    ...
    .action(async (layerOrUrl: string) => {
  ...
  cmd
    ...
    .action(async (layerOrUrl: string, opts: {
      where: string; fields: string; geometry: boolean; crs: string; format: string;
    }) => {
      const url = resolveUrl(layerOrUrl);
  ...
  cmd
    ...
    .option('--crs <crs>', 'Output CRS: 31370 or 4326', '31370')
    .option('-f, --format <format>', 'Output format: geojson|json', 'geojson')
    .action(async (beheerplanId: string, opts: { crs: string; format: string }) => {
...
function printLayerList(layers = MAPSERVER_LAYERS): void {
  const table = new Table({
