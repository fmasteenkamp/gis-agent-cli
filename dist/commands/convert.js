...
import { toWkt, featureCollectionToKml, transformFeatureCollection } from '../utils/geometry.js';
export function buildConvertCommand() {
    return new Command('convert')
        ...
        if (opts.crs) {
            ...
            fc = transformFeatureCollection(fc, toCrs);
        }
        const collection = fc;
        switch (format) {
            case 'kml': {
            ...
            case 'wkt': {
                for (const f of collection.features) {
                    const name = (f.properties?.capaKey ?? f.properties?.CAPAKEY ?? f.properties?.name ?? '');
