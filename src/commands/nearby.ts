...
import { wgs84ToLambert72, lambert72ToWgs84, featureCollectionToKml, transformFeatureCollection, toGoogleMapsUrl } from '../utils/geometry.js';
...
export function buildNearbyCommand(): Command {
  const nearby = new Command('nearby')
    ...
    .action(async (layer: string, location: string, radiusStr: string, opts: { format: string; max: string; crs: string; year?: string; list?: boolean }) => {
      ...
      try {
        ...
        const geom0 = fc.features[0]?.geometry as { type?: string } | undefined;
        if (geom0?.type === 'Point') {
          head.push(
            chalk.bold(crs === '4326' ? 'Lat' : 'X'),
            chalk.bold(crs === '4326' ? 'Lon' : 'Y'),
          );
        }
